...
| Dense posting Boolean kernels: `internal/bitset/ops_simd.go` | Apply four independent 128-bit `AND`, `OR`, or `AND-NOT` operations per loop without assembly. | Public wrappers first size every input/output window; the vector body runs only with eight complete words remaining, uses offsets within those slices, and hands the tail to checked scalar indexing. Exact `dst == a[:0]` or `dst == b[:0]` aliasing loads every vector before its store. | Kernels retain no pointers. Sources and destination remain ordinary caller-owned slices visible to the collector for the call. | `internal/bitset/ops_test.go`, portable/SIMD differential, race, `-d=checkptr=2` | `BenchmarkBoolean`; SIMD must beat the unrolled scalar reference end-to-end on native word storage with unchanged `B/op`/`allocs/op` |
...
- `internal/bitset/ops_simd.go` — `andNotWords`
- `internal/bitset/ops_simd.go` — `andWords`
- `internal/bitset/ops_simd.go` — `orWords`
