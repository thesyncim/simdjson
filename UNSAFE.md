...
| Store page I/O: `internal/storeio/ring_linux.go` | Drive registered fixed-buffer page reads, writes, and data-integrity barriers through Linux `io_uring` without cgo or Go-heap payload buffers. | Compile-time and Linux tests fix every UAPI structure size and critical offset. Kernel-returned ring offsets, dimensions, alignment, drop counters, overflow counters, completion tokens, file indexes, buffer indexes, lengths, and offsets are checked before use. | One locked writer thread owns the ring. Kernel-retained data buffers are anonymous mappings outside the Go heap; Go-backed setup, probe, file, and iovec arrays survive their copying syscalls through `runtime.KeepAlive`. A registered buffer cannot be submitted twice and must not be touched until its completion is consumed. | `internal/storeio/ring_linux_test.go`, Linux architecture cross-builds, race, `-d=checkptr=2` | Ring setup is outside steady state; preparation and completion consume bounded preallocated slots with zero heap allocation. End-to-end page-I/O and portable-fallback benchmarks gate Store integration. |
