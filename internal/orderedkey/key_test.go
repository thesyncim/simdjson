...
var benchmarkKey []byte

func BenchmarkAppendNumber(b *testing.B) {
	for _, number := range []string{
		"42",
		"-123456.7500e-2",
		"9007199254740993123456789.125",
	} {
		b.Run(number, func(b *testing.B) {
			source := []byte(number)
			dst := make([]byte, 0, len(source)+16)
			b.ReportAllocs()
			b.SetBytes(int64(len(source)))
			for i := 0; i < b.N; i++ {
				var ok bool
				dst, ok = AppendNumber(dst[:0], source, Ascending)
				if !ok {
					b.Fatal("encode")
				}
			}
...
func BenchmarkAppendJSONString(b *testing.B) {
	for _, text := range []string{
		...
		b.Run(text, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				var ok bool
				dst, ok = AppendJSONString(dst[:0], source, Ascending)
				if !ok {
					b.Fatal("encode")
				}
			}
