...
func TestAuditReplaceNullReuse(t *testing.T) {
	...
	if err := dec.Decode([]byte(`[1,null,2]`), &reused); err != nil { t.Fatal(err) }
	// Replace contract: reused == fresh. Fresh decode of [1,null,2] is [1,0,2].
	var fresh []int64
	if err := dec.Decode([]byte(`[1,null,2]`), &fresh); err != nil { t.Fatal(err) }
