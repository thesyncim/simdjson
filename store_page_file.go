...
func (s *Store) WritePageFile(file *os.File, options StorePageWriteOptions) (int64, error) {
	...
	keyEntries := make([]storeio.KeyLocation, 0, state.count)
	...
	for chunkID := uint32(0); chunkID < state.chunks.count; chunkID++ {
		...
		for live := chunk.live; live != 0; live &= live - 1 {
			...
			keyEntries = append(keyEntries, storeio.KeyLocation{
	...
	slices.SortFunc(keyEntries, func(a, b storeio.KeyLocation) int {
		if a.Hash < b.Hash {
			return -1
		}
		if a.Hash > b.Hash {
	...
	for _, plan := range keyPlans {
		...
		var page []byte
		if plan.level == 0 {
			page, err = storeio.EncodeKeyLeafPage(scratch[:storePageQuantum], header, plan.leaf,
				fileEnd, nextLogicalID, state.chunks.count, uint32(state.options.ChunkDocuments))
		} else {
			page, err = storeio.EncodeKeyBranchPage(scratch[:storePageQuantum], header, plan.branches, fileEnd, nextLogicalID)
		}
		if err != nil {
			return 0, err
		}
