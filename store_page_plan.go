...
type storeKeyPagePlan struct {
	...
	minHash  uint64
	maxHash  uint64
	leaf     []storeio.KeyLocation
	branches []storeio.KeyBranch
...
func planStoreKeyDirectories(entries []storeio.KeyLocation, generation uint64, nextLogical, offset *uint64) ([]storeKeyPagePlan, storeio.PageRef) {
	if len(entries) == 0 {
		return nil, storeio.PageRef{}
	}
	...
	for levelEnd-levelStart > 1 {
		...
		for start := levelStart; start < levelEnd; start += branchCapacity {
			...
			branches := make([]storeio.KeyBranch, end-start)
			for i := start; i < end; i++ {
				branches[i-start] = storeio.KeyBranch{MaxHash: plans[i].maxHash, Child: plans[i].ref}
			}
			ref := storeio.PageRef{
				Offset: *offset, LogicalID: *nextLogical, Generation: generation,
				Length: storePageQuantum, Kind: storeio.PageKeyDirectory,
