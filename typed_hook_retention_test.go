...
// later call dereferences it. On the safe build the dispatch nils the
...
// This test proves the debug/-race retention trap fires; it is compiled only
// under the safe tag.
...
func TestHookRetentionTrapPanics(t *testing.T) {
	...
	defer func() {
		if recover() == nil {
			t.Fatal("using a retained DecodeCursor after the call should panic in the safe build")
		}
	}()
