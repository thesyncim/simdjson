...
//
// The retention trap. dispatchDecodeHook clears the Cursor's inner pointer once
// the body returns. A body that stashed the *Cursor and dereferences it later
// hits a nil pointer and panics deterministically, instead of silently reading
// a reused or freed frame. That converts a no-retention-contract violation from
// silent corruption (which -race alone does not reliably surface here) into an
// immediate, attributable crash.
...
// properties at once:
//
//   - Corruption safety. The heap Cursor box points at the heap cursor copy, so
//     there is never a heap object holding a pointer into a stack frame — the
//     "bad pointer in Go heap" hazard the itab path must avoid. (The copy's src
//     slice header aliases the same caller-owned backing array, which is not on
//     any goroutine stack.)
...
//   - Retention trap. The Cursor box is heap-resident and its inner pointer is
//     nil'd after the body returns, so a retained *Cursor traps on a nil deref
//     rather than aliasing a reused frame.
func dispatchDecodeHook(hook UnmarshalerSimd, cursor *decoderCursor) error {
	...
	*heapCursor = *(*decoderCursor)(noescape(unsafe.Pointer(cursor)))
	c := &Cursor{d: heapCursor}
	err := hook.UnmarshalSimdJSON(c)
	c.d = nil // trap a retained *Cursor: later use panics on nil deref.
