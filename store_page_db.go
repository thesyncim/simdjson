...
const (
	...
	storePageDBKeyLeafCapacity   = (int(storePageQuantum) - storeio.PageHeaderSize - storeio.PageTrailerSize -
		storeio.KeyDirectoryPayloadHeaderSize) / storeio.KeyLeafEntrySize
	storePageDBKeyBranchCapacity = (int(storePageQuantum) - storeio.PageHeaderSize - storeio.PageTrailerSize -
		storeio.KeyDirectoryPayloadHeaderSize) / storeio.KeyBranchEntrySize
...
type storePageDBKeyLeaf struct {
	...
	entries  [storePageDBKeyLeafCapacity]storeio.KeyLocation
	count    int
	rank     int
	newCount int
	newRef   storeio.PageRef
...
type storePageDBKeyBranchNode struct {
	header   storeio.KeyDirectoryHeader
	entries  [storePageDBKeyBranchCapacity]storeio.KeyBranch
	count    int
	rank     int
	newCount int
	newRef   storeio.PageRef
...
func (db *StorePageDB) mutate(key string, src []byte, deleting bool) error {
	...
	if deleting {
		keyDepth, rewriteKey, err = db.loadKeyDeletePath(pages, root, compiled.hash, storeio.KeyLocation{
...
func (db *StorePageDB) loadKeyDeletePath(pages *storeio.PageFile, root storeio.StateRoot,
	hash uint64, location storeio.KeyLocation) (depth int, rewrite bool, err error) {
	...
	for ref != (storeio.PageRef{}) {
		...
		view := storeio.AdmittedKeyDirectoryPage(lease.Bytes())
		header := view.Header()
...
func (db *StorePageDB) commitMutation(root storeio.StateRoot, oldFileEnd uint64, chunkID uint32,
	...
	if rewriteKey {
		...
		if leaf.count != 0 {
			...
			page, encodeErr := storeio.EncodeKeyLeafPage(buffer[:storePageQuantum], header,
		...
		for level := keyDepth - 1; level >= 0; level-- {
			...
			page, encodeErr := storeio.EncodeKeyBranchPage(buffer[:storePageQuantum], header,
